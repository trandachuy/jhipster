...
@RestController
@RequestMapping("/api/jhipster-example-kafka")
public class JhipsterExampleKafkaResource {

    private final Logger log = LoggerFactory.getLogger(JhipsterExampleKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public JhipsterExampleKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
